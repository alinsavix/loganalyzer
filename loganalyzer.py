...
def checkWindowsVer(lines):
    ...
    if verinfo["version"] == "10.0" and "release" not in verinfo:
        msg = "You are running an unknown Windows 10 release (build %d), which means you are probably using an 'insider' build. Some checks that are applicable only to specific Windows versions will not be performed. Also, becaue insider builds are test versions, you may have problems that would not happen with release versions of windows." % (
            verinfo["build"])
